...
export function Sidebar() {
  ...
  return (
    <>
    <aside className="w-64 min-h-screen bg-zinc-950 border-r border-white/5 flex flex-col p-4 sticky top-0">
      {/* Logo */}
      ...
      <div className="mt-auto space-y-3">
        ...
        <div 
          onClick={logout}
          className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
        >
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold">
            {role === "RECRUITER" ? "R" : "C"}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate">{role === "RECRUITER" ? "Recruiter" : "Candidate"}</p>
